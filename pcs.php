...
$perPageRaw = $_GET["per_page"] ?? 50;
$showAll    = ($perPageRaw === 'all');
$perPage    = $showAll ? 0 : (in_array((int)$perPageRaw, $allowedPerPage, true) ? (int)$perPageRaw : 50);
...
$totalPages = $showAll ? 1 : (int)ceil($total / $perPage);
...
if ($showAll) {
  $sql = "SELECT id, hostname, serial, marque, modele, utilisateur, os, os_version, architecture, domaine, statut, updated_at
          FROM pcs $where ORDER BY $sortCol $dir";
} else {
  $sql = "SELECT id, hostname, serial, marque, modele, utilisateur, os, os_version, architecture, domaine, statut, updated_at
...
<div class="container-fluid py-4">
  ...
  <div class="card shadow-sm">
    <div class="card-header d-flex justify-content-between align-items-center gap-2 py-2 flex-wrap">
      <div class="d-flex align-items-center gap-2">
        <label for="perPageSelect" class="small text-muted mb-0">Afficher</label>
        <select id="perPageSelect" class="form-select form-select-sm" style="width:auto;">
          <?php foreach ($allowedPerPage as $pp): ?>
            <option value="<?= $pp ?>" <?= (!$showAll && $perPage === $pp) ? "selected" : "" ?>><?= $pp ?> par page</option>
          <?php endforeach; ?>
          <option value="all" <?= $showAll ? "selected" : "" ?>>Tout afficher</option>
        </select>
      </div>
      <?php if ($can_bulk): ?>
  ...
  <p class="text-center text-muted small mt-3">
    <?= $total ?> PC au total — tout affiché
  </p>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (perPageSel) {
    perPageSel.addEventListener('change', () => {
      ...
      u.searchParams.set('per_page', perPageSel.value);
